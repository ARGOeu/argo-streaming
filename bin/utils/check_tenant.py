...
def get_date_days_back(date_str, days_back):
    ...
    target_date = datetime.strptime(
        date_str, '%Y-%m-%d') - timedelta(days=days_back)
...
def check_tenant_hdfs(tenant, target_date, days_back, namenode, hdfs_user, client, config):
    ...
    hdfs_metric = config.get("HDFS", "path_metric").fill(
        namenode=namenode.geturl(), user=hdfs_user, tenant=tenant)
    hdfs_status = dict()
    ...
    hdfs_sync = config.get("HDFS", "path_sync").fill(
        namenode=namenode.geturl(), user=hdfs_user, tenant=tenant)
    sync_list = ["metric_profile", "group_groups",
                 "group_endpoints", "weights", "downtimes"]
    ...
    for report in reports:
        ...
        for item in sync_list:
            ...
            for days_iter in range(0, days_back):
                ...
                sync_path = "".join(
                    [hdfs_sync.path, "/", report, "/", item, "_", check_date, ".avro"])
                try:
        ...
        for item in list(report_profiles.keys()):
            profile_path = "".join(
                [hdfs_sync.path, "/", report_profiles[item].format(tenant, report)])
            try:
...
def check_tenant_ams(tenant, target_date, ams, config):
    ...
    if ams.check_project_exists(tenant):
        ...
        ams_tenant["metric_data"]["messages_arrived"] = ams.get_topic_num_of_messages(
            tenant, "metric_data")
        ams_tenant["sync_data"]["messages_arrived"] = ams.get_topic_num_of_messages(
            tenant, "sync_data")
...
def check_tenants(tenants, target_date, days_back, config):
    ...
    if not tenant_uuids:
        log.error(
            "Without tenant uuids service is unable to check and upload tenant status")
        sys.exit(1)
    ...
    for tenant in tenants:
        ...
        # get hdfs status
        status_tenant["hdfs"] = check_tenant_hdfs(
            tenant, target_date, days_back, namenode, hdfs_user, client, config)
        # get ams status
        status_tenant["ams"] = check_tenant_ams(
            tenant, target_date, ams, config)

        log.info("Status for tenant[{}] = {}".format(
            tenant, json.dumps(status_tenant)))
        # Upload tenant status to argo-web-api
        complete_status.append(status_tenant)
        upload_tenant_status(api_endpoint, api_token, tenant,
                             tenant_uuids[tenant], status_tenant)
...
def upload_tenant_status(api_endpoint, api_token, tenant, tenant_id, tenant_status):
    ...
    log.info("Uploading status for tenant: {}({}) at api: {}".format(
        tenant, tenant_id, api_endpoint))
    url = "https://{}/api/v2/admin/tenants/{}/status".format(
    ...
    r = requests.put(url, headers=headers, data=json.dumps(
        tenant_status), verify=False)
    if 200 == r.status_code:
        log.info("Tenant's {} status upload succesfull to {}".format(
            tenant, api_endpoint))
        return True
    else:
        log.error("Error uploading to the api {}, status_code: {} - response: {}".format(
            api_endpoint, r.status_code, r.text))
        return False
