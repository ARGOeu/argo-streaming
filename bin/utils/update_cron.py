...
def gen_tenant_all(config, tenant, reports=None, user=None):
    ...
    for report in reports:
        ...
        cron_body = cron_body + \
            gen_batch_ar(config, tenant, report, "hourly",
                         user, mongo_method) + "\n"
        # Generate daily batch Ar
        cron_body = cron_body + \
            gen_batch_ar(config, tenant, report, "daily",
                         user, mongo_method) + "\n"
        # Generate hourly batch Status
        cron_body = cron_body + \
            gen_batch_status(config, tenant, report, "hourly",
                             user, mongo_method) + "\n"
        # Generate daily batch Status
        cron_body = cron_body + \
            gen_batch_status(config, tenant, report, "daily",
                             user, mongo_method) + "\n"
    return cron_body + "\n"
...
def update_cron_tab(cron_gen_data):
    ...
    date_stamp = "#The rest are generated by argo engine at {}".format(
        str(datetime.utcnow()))
    cron_data = cron_data + "\n" + date_stamp + "\n\n" + cron_gen_data
