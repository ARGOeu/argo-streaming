...
CONF_FILE = os.path.abspath(os.path.join(
    os.path.dirname(__file__), '../../conf/argo-streaming.conf'))
SCHEMA_FILE = os.path.join(os.path.dirname(
    __file__), '../../conf/config.schema.json')

# relative path to ar job submit script
BATCH_AR = os.path.abspath(os.path.join(
    os.path.dirname(__file__), '../ar_job_submit.py'))
# relative path to status job submit script
BATCH_STATUS = os.path.abspath(os.path.join(
    os.path.dirname(__file__), '../status_job_submit.py'))
...
class TestClass(unittest.TestCase):
    ...
    def test_update_cron(self):
        ...
        self.assertEqual(expected, gen_batch_ar(
            config, "TENANT_A", "report1", "daily", "foo", "upsert"))
        ...
        self.assertEqual(expected, gen_batch_ar(
            config, "TENANT_A", "report1", "hourly"))
        ...
        self.assertEqual(expected, gen_batch_status(
            config, "TENANT_B", "report1", "daily", "foo", "upsert"))
        ...
        self.assertEqual(expected, gen_batch_status(
            config, "TENANT_B", "report1", "hourly"))
