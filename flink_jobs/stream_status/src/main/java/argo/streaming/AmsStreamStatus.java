...
//					kafkaProps);
//			
            FlinkKafkaProducer kSink = new FlinkKafkaProducer(kafkaTopic, new SimpleStringSchema(),
                    kafkaProps);
    ...
    private static class MetricDataWithGroup extends RichCoFlatMapFunction<String, Tuple2<String, String>, Tuple2<String, MetricData>> {
        ...
        public void open(Configuration parameters) throws IOException, ParseException, URISyntaxException {
            ...
            List<MetricProfile> mpsList = Arrays.asList(amr.getListMetrics());
            List<GroupEndpoint> egpList = Arrays.asList(amr.getListGroupEndpoints());
...
//			ArrayList<GroupEndpoint> egpListFull = (ArrayList<GroupEndpoint>)(Arrays.asList(amr.getListGroupEndpoints()));
            ...
            ArrayList<Downtime> downList = new ArrayList<Downtime>();
            Collections.addAll(downList, amr.getListDowntimes());
      
            ArrayList<MetricProfile> mpsList = new ArrayList<MetricProfile>();
            Collections.addAll(mpsList, amr.getListMetrics());
      
            ArrayList<GroupEndpoint> egpListFull = new ArrayList<GroupEndpoint>();
            Collections.addAll(egpListFull, amr.getListGroupEndpoints());
