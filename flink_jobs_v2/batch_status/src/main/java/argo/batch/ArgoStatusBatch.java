 ...
 *
 * Submit job in flink cluster using the following parameters --mps: path
 * tometric profile sync file (For hdfs use: hdfs://namenode:port/path/to/file)
 * --egp: path to endpoints group topology file (For hdfs use:
 * hdfs://namenode:port/path/to/file) --ggp: path to group of groups topology
 * file (For hdfs use: hdfs://namenode:port/path/to/file) --pdata: path to
 * previous day's metric data file (For hdfs use:
 * hdfs://namenode:port/path/to/file) --mdata: path to metric data file (For
 * hdfs use: hdfs://namenode:port/path/to/file) --ops: path to operations
 * profile file (For hdfs use: hdfs://namenode:port/path/to/file) --aps: path to
 * aggregations profile file (For hdfs use: hdfs://namenode:port/path/to/file)
 * --cfg: path to report's configuration file (For hdfs use:
 * hdfs://namenode:port/path/to/file) --rec: path to recomputations file
 * --run.date: target date of computation in DD-MM-YYYY format --mongo.uri: path
 * to MongoDB destination (eg mongodb://localhost:27017/database.table
...
public class ArgoStatusBatch {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (params.has("api.proxy")) {
            amr.getRequestManager().setProxy(params.get("api.proxy"));
        }
