...
public class ApiResourceManager {
    ...
    private void getDowntimesFromWebApi() throws UnknownHostException {
        ...
        data.put( ApiResource.DOWNTIMES, apiResponseParser.getJsonData(content, false)
        );
    }
    ...
    public void getRemoteFeedTopologyIsExternal() throws UnknownHostException {
        ...
        String fullURL = String.format(
                path,
                this.argoMonApiInitializer.getArgoMonApiEndpoint(),
                this.tenant
        );
    ...
    public Downtime[] getListDowntimes() {
        ...
        if (!this.data.containsKey(ApiResource.DOWNTIMES) || this.data.get(ApiResource.DOWNTIMES)==null) {
            Downtime[] rArr = new Downtime[results.size()];
           return  results.toArray(rArr);
        }
...
//            boolean isExternal = Boolean.parseBoolean(this.data.get(ApiResource.FEEDTOPOLOGY));
            String feedTopology = this.data.get(ApiResource.FEEDTOPOLOGY);

            if (feedTopology == null) {
                throw new IllegalStateException("Feed topology information is not available");
            }

            boolean isExternal = Boolean.parseBoolean(feedTopology);
