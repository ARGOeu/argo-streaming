...
public class FillMissing extends RichGroupReduceFunction<MetricData, StatusMetric> {
    ...
    /**
     * Iterates over all metric data and gathers a set of encountered service
     * endpoint metrics. Then subtracts it from a set of expected service
     * endpoint metrics (based on topology) so as the missing service endpoint
     * metrics to be identified. Then based on the list of the missing service
     * endpoint metrics corresponding metric data are created
     *
     * @param in An Iterable collection of MetricData objects
     * @param out A Collector list of Missing MonData objects
     */
    @Override
