...
public class ArgoApiSource extends RichSourceFunction<Tuple2<String, String>> {
    ...
    private transient ApiResourceManager client;  // Mark as transient
    // private final ApiResourceManager initialClient; // Store clientA reference
    private Instant timeSnapshot = null;
    private EnumMap<ApiResource, String> apiData = new EnumMap<>(ApiResource.class);
    ...
    public int getCheckApiInterval() {
        return checkApiInterval;
    }

    public EnumMap<ApiResource, String> getApiData() {
        return apiData;
    }

    public void setApiData(EnumMap<ApiResource, String> apiData) {
        this.apiData = apiData;
    }

    public void setCheckApiInterval(int checkApiInterval) {
        this.checkApiInterval = checkApiInterval;
    }
    ...
    public void run(SourceContext<Tuple2<String, String>> ctx) throws Exception {
        ...
        while (isRunning) {
            ...
            if (isTimeForSync(td) || shouldCheckApi(td)) {
                ...
                if (!this.client.isShouldRepeat()) {
                    apiData = this.client.getData();
                    AmsStreamStatus.setApiData(apiData);
                }
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        this.client.setData(this.apiData);
        this.client.setReportID(this.reportID);
        this.client.setVerify(this.verify);
        if (this.useProxy) {
    ...
    private boolean shouldCheckApi(Duration td) {
        if (td.toMinutes() > checkApiInterval) {

            if (this.client.isShouldRepeat()) {
                this.client.setShouldRepeat(false);
                return true;
            } else {
                return false;
